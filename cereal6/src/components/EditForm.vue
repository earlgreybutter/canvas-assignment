    ...
    <div style="padding: 0 15px 0 10px; font-size: 14px; width: 200px; margin-top: 2em">
        ...
        <div v-if="cellData && cellData.value">
            ...
            <div style="display: block;padding-top: 5px; align-items: baseline">
                ...
                <div style="flex: auto">
                    <input @focus="$event.target.select()" type="radio" name="showNhide" value=true @input="debounce_change"
                           v-model="cellData.value.showNhide"/>show
                    <input @focus="$event.target.select()" type="radio" name="showNhide" value=false @input="debounce_change"
                           v-model="cellData.value.showNhide"/>hide
                </div>
            </div>
        </div>
